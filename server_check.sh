...
grade_color() {
    local mbps_int=$1
    if   [ "$mbps_int" -ge 500 ]; then echo "$GREEN";  echo "极速 🚀"
    elif [ "$mbps_int" -ge 100 ]; then echo "$GREEN";  echo "优秀"
    elif [ "$mbps_int" -ge 30  ]; then echo "$CYAN";   echo "良好"
    elif [ "$mbps_int" -ge 5   ]; then echo "$YELLOW"; echo "一般"
    else                               echo "$RED";    echo "较慢"; fi
}
...
for i in "${!DL_URLS[@]}"; do
    ...
    if [ -n "$speed" ] && [ "$speed" != "0" ]; then
        ...
        read bar_color grade <<< $(grade_color "$mbps_int")
        bar=$(draw_speed_bar "$mbps_int" 500)
        printf "  ${GRAY}%-14s${NC}  ${bar_color}%s${NC}  ${WHITE}%8s Mbps${NC}  ${bar_color}%s${NC}\n" \
            "$name" "$bar" "$mbps" "$grade"
        ...
        printf "  ${GRAY}%-14s${NC}  ${RED}%-28s%9s        无法连接${NC}\n" \
            "$name" "░░░░░░░░░░░░░░░░░░░░░░░░░░░░" "—"
    fi
done
