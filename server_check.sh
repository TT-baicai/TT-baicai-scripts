...
# 所有标签列统一在第26列开始输出值（光标定位，与标签宽度无关）
COL_VALUE=26

row() {
    local label="$1" value="$2" color="${3:-$WHITE}"
    printf "  ${GRAY}%s${NC}" "$label"
    printf "\033[${COL_VALUE}G${color}%s${NC}\n" "$value"
}
...
status_row() {
    ...
    printf "  ${GRAY}%s${NC}" "$label"
    printf "\033[${COL_VALUE}G%b ${color}%s${NC}\n" "$icon" "$value"
}
...
printf "  ${GRAY}%s${NC}" "物理内存"
printf "\033[${COL_VALUE}G${bar_color}%s${NC} ${WHITE}%s%%${NC} ${GRAY}(%sMiB / %sMiB)${NC}\n" \
    "$MEM_BAR" "$MEM_PCT" "$MEM_USED" "$MEM_TOTAL"

if [ "$SWAP_TOTAL" -gt 0 ]; then
    SWAP_PCT=$(( SWAP_USED * 100 / SWAP_TOTAL ))
    SWAP_BAR=$(make_bar $SWAP_PCT)
    printf "  ${GRAY}%s${NC}" "Swap"
    printf "\033[${COL_VALUE}G${YELLOW}%s${NC} ${WHITE}%s%%${NC} ${GRAY}(%sMiB / %sMiB)${NC}\n" \
...
while read -r line; do
    pct=$(echo "$line" | awk '{print $5}' | tr -d '%' | awk '{printf "%d", $1+0.5}')
    ...
    printf "  ${GRAY}%s${NC}" "$mount"
    printf "\033[${COL_VALUE}G${dcolor}%s${NC} ${WHITE}%3s%%${NC} ${GRAY}可用:%s / 共:%s${NC}  ${dcolor}[%s]${NC}\n" \
        "$DISK_BAR" "$pct" "$avail" "$size" "$dstatus"
...
# ANSI 光标列定位常量（第N列）
# 布局: "  名称" 从第1列开始，条形图从第20列，速度从第54列，评级从第68列
COL_BAR=20      # 条形图起始列
COL_SPEED=54    # 速度数值起始列
COL_GRADE=68    # 评级起始列

# 打印一行测速结果（使用光标定位，完全与名称宽度无关）
print_speed_row() {
    local name="$1" bar="$2" mbps="$3" grade="$4" bar_color="$5"
    printf "  ${GRAY}%s${NC}" "$name"
    printf "\033[${COL_BAR}G${bar_color}%s${NC}" "$bar"
    printf "\033[${COL_SPEED}G${WHITE}%8s Mbps${NC}" "$mbps"
    printf "\033[${COL_GRADE}G${bar_color}%s${NC}\n" "$grade"
}

print_speed_fail() {
    local name="$1"
    printf "  ${GRAY}%s${NC}" "$name"
    printf "\033[${COL_BAR}G${RED}%s${NC}" "░░░░░░░░░░░░░░░░░░░░░░░░░░░░"
    printf "\033[${COL_SPEED}G${GRAY}%8s Mbps${NC}" "—"
    printf "\033[${COL_GRADE}G${RED}无法连接${NC}\n"
}
...
for i in "${!DL_URLS[@]}"; do
    ...
    if [ -n "$speed" ] && [ "$speed" != "0" ]; then
        ...
        print_speed_row "$name" "$bar" "$mbps" "$grade" "$bar_color"
